...
namespace RobotRampage
{
    static class WeaponManager
    {
        ...
        {
            Particle shot = new Particle(location, Texture, shotRectangle, velocity, Vector2.Zero, 400f, 
                120, Color.White, Color.White);

            shot.AddFrame(new Rectangle(shotRectangle.X + shotRectangle.Width, shotRectangle.Y, 
        ...
        private static void TryToSpawnPowerup(int x, int y, WeaponType type)
        {
            ...
            {
                Sprite newPowerup = new Sprite(new Vector2(thisDestination.X, thisDestination.Y),
                    Texture, new Rectangle(64, 128, 32, 32), Vector2.Zero);
        ...
        private static void CheckPowerupSpawns(float elapsed)
        {
            ...
            if (timeSinceLastPowerup >= timeBetweenPowerups)
            {
                ...
                TryToSpawnPowerup(rand.Next(0, TileMap.MapWidth), rand.Next(0, TileMap.MapHeight), 
                    type);
            }
        }
        #endregion
        ...
        private static void CheckPowerupPickups()
        {
            ...
            {
                if (Player.BaseSprite.IsCircleColliding(PowerUps[x].WorldCenter, 
        ...
        private static void CheckShotEnemyImpacts(Sprite shot)
        {
            ...
            foreach (Enemy enemy in EnemyManager.Enemies)
            {
                if (!enemy.Destroyed)
                {
                    if (shot.IsCircleColliding(enemy.EnemyBase.WorldCenter, 
                        enemy.EnemyBase.CollisionRadius))
                    {
                        ...
                        if(shot.Frame == 0)
                        {
                            EffectsManager.AddExplosion(enemy.EnemyBase.WorldCenter, 
