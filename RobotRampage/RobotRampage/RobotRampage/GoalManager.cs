...
namespace RobotRampage
{
    static class GoalManager
    {
        ...
        public static void Initialize(Texture2D textureSheet, Rectangle initialActiveRectangle,
            Rectangle initialDisabledRectangle, int activeFrames, int disabledFrames)
        {
        ...
        public static void DetectShutdowns()
        {
            foreach (ComputerTerminal terminal in computerTerminals)
            {
                if (terminal.Active)
                {
                    if (terminal.IsCircleColliding(Player.BaseSprite.WorldCenter, 
        ...
        public static void AddComputerTerminal()
        {
            ...
            if (Vector2.Distance(TileMap.GetSquareCenter(startX, startY), Player.BaseSprite.WorldCenter)
            ...
            if (path != null)
            {
                ...
                Sprite activeSprite = new Sprite(new Vector2(squareRect.X, squareRect.Y), texture,
                    initialActiveFrame, Vector2.Zero);

                for (int x = 1; x < 3; x++)
                {
                    activeSprite.AddFrame(new Rectangle(initialActiveFrame.X + 
                        (x * initialActiveFrame.Width), initialActiveFrame.Y, initialActiveFrame.Width,
                        initialActiveFrame.Height));
                }
                activeSprite.CollisionRadius = 15;

                Sprite disabledSprite = new Sprite(new Vector2(squareRect.X, squareRect.Y), texture,
                    initialDisabledFrame, Vector2.Zero);

                ComputerTerminal terminal = new ComputerTerminal(activeSprite, disabledSprite,
